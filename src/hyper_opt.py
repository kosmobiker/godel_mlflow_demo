...
class HyperOpt():
    ...
    """
    def __init__(self, experiment_name, data_path, n_trials, timeout):
        ...
        self.ts = time.strftime("%Y-%m-%d_%H-%M-%S", time.gmtime(time.time()))
    ...
    def preprocess(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        to-do: add logging
        to-do: save preprocessor to MLflow model registry
        """
    ...
    def find_best_parameters(self) -> optuna.trial._frozen.FrozenTrial:
        ...
        study = optuna.create_study(
                        study_name=f"xgboost_cv_optimize_{self.ts}",
                        pruner=pruner,
                        direction="minimize")
        study.optimize(self._objective, n_trials=self.n_trials, timeout=self.timeout, callbacks=[mlflc])
    ...
    def _metrics(self, true: np.array, pred:np.array) -> Tuple[float, float, float, float]:
        ...
        rmse = mean_squared_error(true, pred, squared=True)
    ...
    def evaluate_and_register(self):
        with mlflow.start_run():
            ...
            mlflow.xgboost.log_model(model,
                                    f"model_xgboost_{self.ts}",
                                    registered_model_name="house_pricing_xgboost_model")
            """
            to-do: add logging
            to-do: save model to MLflow model registry
            to-do: save metrics to the description of the model
            """
                        
            
if __name__ == "__main__":
    hyperopt = HyperOpt(EXPERIMENT_NAME, DATA_PATH, N_TRIALS, TIMEOUT)
